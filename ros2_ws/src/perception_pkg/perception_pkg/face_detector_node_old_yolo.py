"""
Face Detector Node (Service-Based)
...
DESCRIPTION:
    ...
    locate faces in low-resolution camera images. This node operates as a service,
    responding to synchronous detection requests from the PersonStateManager.
...
RESPONSIBILITIES:
    ...
    6. Return face detection results synchronously
...
DATA FLOW:
    1. Receive service request (track_id, bbox, timestamp)
    ...
    8. Return face detection result synchronously
...
class FaceDetectorNode(Node):
    ...
    def detect_face_callback(self, request, response):
        ...
        if timestamp_ns not in self.frame_cache:
            ...
            self.get_logger().warning(f"[DETECT] Available timestamps in cache: {list(self.frame_cache.keys())[:5]}")
            return response
        ...
        try:
            ...
            self.get_logger().info(
                f"[DETECT] Track {request.track_id}: ROI original=({orig_x}, {orig_y}, {orig_w}, {orig_h}), " +
                f"clipped=({x}, {y}, {w}, {h})"
            )
            ...
            self.get_logger().info(f"[DETECT] Track {request.track_id}: Found {len(faces)} faces in ROI")
    ...
    def postprocess_two_outputs(self, scores: np.ndarray, boxes: np.ndarray, original_shape: tuple) -> list:
        ...
        # Debug logging
        self.get_logger().info(f"[POSTPROCESS] Total detections: {len(face_scores)}")
        self.get_logger().info(f"[POSTPROCESS] Score range: min={face_scores.min():.4f}, max={face_scores.max():.4f}, mean={face_scores.mean():.4f}")
        self.get_logger().info(f"[POSTPROCESS] Confidence threshold: {self.conf_threshold}")

        # Show top 10 scores
        top_10_indices = np.argsort(face_scores)[-10:][::-1]
        self.get_logger().info(f"[POSTPROCESS] Top 10 scores: {face_scores[top_10_indices]}")
        ...
        self.get_logger().info(f"[POSTPROCESS] After confidence filter: {len(filtered_boxes)} detections")
